...
The caster provides *localized* corrections, so the client periodically reports the vehicle's position to it. After a continuous ≥2D fix for a few seconds, GPS_RAW_INT is converted to an NMEA-GGA sentence and sent to the caster every 10 seconds. If the fix is lost while the receiver stays up, the last-known position keeps being reported — downgraded to plain-GPS quality — for a few minutes: the caster drops a session whose GGA goes quiet, and holding it open keeps corrections flowing through a receiver cold start. MGA forwarding itself is not gated on fix — only on the receiver being up.
