...
GPS_RAW_INT is also the receiver-presence signal. PX4 publishes a report only when it parses one, so the stream stops outright when the receiver drops off the bus or the driver goes back to configuring it — which is distinct from a receiver that is up and reporting `fix_type` 0. If it stops for 3 seconds the client says so and closes the NTRIP connection: PX4 drops injected data until the driver has the receiver configured again, so nothing sent during that window arrives anyway. When GPS_RAW_INT resumes, the client reports it, reconnects, and the fix state starts over. That reconnect is also how the receiver gets assistance again — it came back without ephemeris, and the caster only delivers the AssistNow burst at connect.
